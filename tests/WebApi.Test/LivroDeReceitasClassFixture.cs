...
namespace WebApi.Test
{
    public class LivroDeReceitasClassFixture : IClassFixture<CustomWebApplicationFactory>
    {
        ...
        {
            if (token.NotEmpty().IsFalse())
                return;

            _httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);
        }

        private static void AddListToMultipartContent(MultipartFormDataContent multipartContent, string propertyName, System.Collections.IList list)
        {
        ...
        private static void AddClassListToMultipartContent(MultipartFormDataContent multipartContent, string propertyName, System.Collections.IList list)
        {
            var index = 0;
            ...
            foreach (var item in list)
            {
                ...
                foreach (var property in classProperties)
                {
                    ...
                    multipartContent.Add(new StringContent(propertyValue!.ToString()!), $"{propertyName}[{index}][{property.Name}]");
                }
